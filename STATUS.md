...
| `[env.NAME]` | `default` lets the host's value win; `fixed` ignores the host; `ask` prompts once and keeps the answer host-side for every box; `required` refuses a start without a value; `secret` masks it |
