...
| `wormhole broker` — the host-side proxy on a unix socket. Holds the credential, renews it under a lock, strips the box's dummy key and injects the real token, and streams the reply straight back. A box with `[access] broker = true` gets the socket bound read-only under `/run` plus the forwarder written there at start — its own crate (`wormhole-forward`), compiled static against musl by `build.rs` and carried inside wormhole's binary, so it runs in any image regardless of the image's libc — and the agent is pointed at it by `ANTHROPIC_BASE_URL`, so the box holds no credential and needs no route. `tests/forwarder.rs` holds the embedded helper to no `PT_INTERP` and relays through it for real | `cargo run -p wormhole -- broker` |
