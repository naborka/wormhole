...
Tests today: **496** (`cargo test`: 393 pure, 103 binary), **31 kernel** (`cargo test -p wormhole --features kernel-tests`, real host only). The 17 in `tests/build.rs` fetch a rootfs through `curl`, so they need one on the host; everything else runs anywhere.
...
| `wormhole broker` — the host-side proxy on a unix socket. Holds the credential, renews it under a lock, strips the box's dummy key and injects the real token, and streams the reply straight back. A box with `[access] broker = true` gets the socket and wormhole's own binary bound read-only under `/run`, starts a forwarder on its loopback, and is pointed at it by `ANTHROPIC_BASE_URL` — so the box holds no credential and needs no route. Brokering into an image requires a statically linked wormhole (no `PT_INTERP`): a dynamic binary is refused at launch with the musl rebuild command, instead of the forwarder dying inside the box with a bare "not found" | `cargo run -p wormhole -- broker` |
